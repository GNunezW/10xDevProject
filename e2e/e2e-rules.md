...
- Use storageState for authentication — never log in through UI
  in individual tests (auth.setup.ts is the one exception).
- Name the test after a risk from context/foundation/test-plan.md.
- The assertion must fail if that risk materializes.
